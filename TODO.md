# Short-term Actionable Tasks

## High Priority
~~1. **Modernize Boost Filesystem Usage**: Update deprecated Boost Filesystem v2 calls (e.g., `branch_path()`) to modern v3 equivalents (e.g., `parent_path()`) to eliminate compilation warnings and prepare for newer Boost versions.~~ (Completed)
~~2. **Settings GUI Linkage**: Ensure the `wxWidgets` settings GUI compiles and integrates cleanly alongside the main executable without configuration errors.~~ (Completed)
~~3. **Verify Rendering Pipeline**: Test the OpenGL rendering path on a machine with a display context to ensure textures are correctly uploaded via the modernized FFmpeg decoding pipeline.~~ (Completed)

## Medium Priority
~~4. **Remove Unused GLEW Dependencies**: Fully remove any lingering system `libGLEW` linking flags if they are no longer required due to the `GLee` reversion.~~ (Completed)
~~5. **Code Cleanup**: Address `register` storage class warnings generated by modern GCC implementations on older C code (`Common/isaac.cpp`).~~ (Completed)

## Low Priority
~~6. **Documentation**: Add inline comments to the newly updated FFmpeg decoding steps explaining the transition from the old packet/frame API.~~ (Completed)

## HUD & GUI Phase 2 Tasks
~~7. **HUD Data Linkage**: Update `Client/client.h` to pull frame data values from the core decoder struct to render metrics visually via the OSD logic.~~ (Completed)
~~8. **ImGui Prototyping (Research)**: Scope out a potential `Dear ImGui` rendering integration inside `DisplayOutput/RendererGL.cpp` to replace external settings configurations natively.~~ (Completed)
9. **ImGui Implementation**: Add Dear ImGui to the project, update CMake, and replace the legacy `Hud.cpp` rendering mechanisms.

## Network Modernization Phase 3 Tasks
10. **Async Networking (Boost.Asio)**: Refactor `ContentDownloader/SheepDownloader.cpp` and `SheepGenerator.cpp` to remove aggressive thread halting (`boost::thread::sleep`) when `curl_multi_perform` encounters failure states. Transition to an event-driven `Boost.Asio` architecture to prevent global render stutter.
